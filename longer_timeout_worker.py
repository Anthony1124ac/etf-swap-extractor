...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('longer_timeout_worker.log'),
...
def process_ticker_with_timeout(extractor, ticker, cik, series_id, timeout_seconds=300):
    """Process a ticker with longer timeout protection (5 minutes)"""
    # Set up timeout
...
def main():
    """Main function for worker with longer timeouts"""
    logger.info("=" * 80)
    logger.info("LONGER TIMEOUT ETF BATCH LOADER WORKER STARTING")
    logger.info("=" * 80)
    ...
    # Process tickers with longer timeout protection
    for i, (ticker, cik, series_id) in enumerate(zip(tickers, ciks, series_ids), 1):
        ...
        try:
            ...
            success = process_ticker_with_timeout(extractor, ticker, cik, series_id, timeout_seconds=300)
