...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fixed_worker_batch_load.log'),
        logging.StreamHandler(sys.stdout)  # Also log to stdout for Render logs
    ]
)
logger = logging.getLogger(__name__)

def main():
    """Main function for background worker batch processing with loop prevention"""
    logger.info("=" * 80)
    logger.info("FIXED ETF BATCH LOADER WORKER STARTING")
    logger.info("=" * 80)
    ...
    # Process tickers with better error handling and loop prevention
    for i, (ticker, cik, series_id) in enumerate(zip(tickers, ciks, series_ids), 1):
        ...
        try:
            ...
            # Process the ticker with timeout protection
